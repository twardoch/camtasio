...
class TestFrameStamp:
    ...
    def test_framestamp_to_frame_rate(self):
        ...
        assert abs(fs_24.total_seconds - fs.total_seconds) < 1e-10  # Same duration (within rounding)
...
class TestFrameStampArithmetic:
    ...
    def test_arithmetic_with_zero(self):
        ...
        result = zero - fs
        assert result.frame_number == -100  # Negative frame number is allowed in result
        assert result.frame_rate == 30
...
class TestFrameStampInvalidOperations:
    ...
    def test_comparison_with_non_framestamp(self):
        ...
        assert (fs < "not a framestamp") is NotImplemented
        assert (fs <= 123) is NotImplemented
        assert (fs > 45.6) is NotImplemented
        assert (fs >= None) is NotImplemented
    ...
    def test_negative_result_from_subtraction(self):
        """Test that subtraction can result in negative frame numbers."""
        ...
        assert result.frame_rate == 30
        # Note: The resulting FrameStamp with negative frame number 
        # might not be directly constructible, but is valid as a calculation result