...
pytestmark = pytest.mark.skip(
    reason="Track and media management not yet implemented in unified package"
)
...
class TestTrackMedia:
    ...
    def test_add_overlapping_media_raises_value_error(self, project, media_root):
        ...
        track.medias.add_media(project.media_bin.import_media(media_root / "llama.jpg"), 0)

        # Add another image at the same place
        with pytest.raises(ValueError):
            track.medias.add_media(project.media_bin.import_media(media_root / "monkey.jpg"), 0)

    def test_add_non_overlapping_media(self, project, media_root):
        track = project.timeline.tracks.insert_track(2, "test-track")

        # Add image at start of track
        media1 = track.medias.add_media(project.media_bin.import_media(media_root / "llama.jpg"), 0)

        # Add another image at the same place
        track.medias.add_media(
            project.media_bin.import_media(media_root / "monkey.jpg"),
            media1.start + media1.duration,
        )
    ...
    def test_add_media_with_default_chromakey_effect(self, project, media_root):
        ...
        bin_media = project.media_bin.import_media(media_path)
        track = project.timeline.tracks.insert_track(2, "test-track")
        media_id = track.medias.add_media(bin_media, 0, effects=[ChromaKeyEffect()]).id
        media = track.medias[media_id]
        assert media.id == media_id
        assert media.source == bin_media.id
    ...
    def test_get_track_media_effects(self, project, media_root):
        ...
        media_id = track.medias.add_media(bin_media, 0, effects=[effect]).id
        media = track.medias[media_id]
